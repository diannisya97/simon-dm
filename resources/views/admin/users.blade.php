...
<section
    ...
    <div class="absolute top-0 bg-[#0BB4A6] w-full px-7 py-[18px] flex flex-row items-center justify-between">
        ...
        <div class="flex flex-col">
            <h1 class="text-base text-center font-bold text-black ml-2">Data Verifikasi</h1>
        </div>
    ...
    <div class="flex flex-col items-start w-full mt-4 px-7 gap-2">
        @foreach ($users as $user)
        @php
            $catatanKesehatan = \App\Models\CatatanKesehatan::where('user_id', $user->id)->latest()->first();
            $gulaDarah = $catatanKesehatan ? $catatanKesehatan->gula : null;

            if ($gulaDarah === null) {
                $statusDiabetes = 'Data Gula Tidak Tersedia';
            } elseif ($gulaDarah < 140) {
                $statusDiabetes = 'Pasien Non Diabetes';
            } elseif ($gulaDarah < 200) {
                $statusDiabetes = 'Pasien Waspada';
            } else {
                $statusDiabetes = 'Pasien Diabetes';
            }
        @endphp
        <div class="flex flex-row justify-between items-center w-full border-b-[2px] border-slate-300 py-1">
            <div class="flex flex-col justify-center items-start">
                <p class="text-xs font-semibold">{{ $user->name }}
                    <span class="{{ $user->role == 0 ? 'text-green-500' : 'text-red-500' }} text-[0.7rem]">
                    {{ $user->role == 0 ? '(Terverifikasi)' : '(Perlu Verifikasi)' }}
                </span>
                </p>
                <p class="text-xs">{{ $statusDiabetes }}</p>
                <p class="text-xs">{{ Carbon::parse($user->tanggal_lahir)->translatedFormat('d F Y') }}</p>
                <p class="text-xs">{{ ($user->tanggal_lahir)->age }} Tahun</p>
            </div>
            ...
            <div class="flex flex-row items-center justify-center gap-2">
                ...
                @else
                    <button class="flex flex-row items-center justify-center" onclick="verifikasiUser({{ $user->id }})">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M10 20C4.477 20 0 15.523 0 10C0 4.477 4.477 0 10 0C15.523 0 20 4.477 20 10C20 15.523 15.523 20 10 20ZM8.823 12.14L6.058 9.373L5 10.431L8.119 13.552C8.30653 13.7395 8.56084 13.8448 8.826 13.8448C9.09116 13.8448 9.34547 13.7395 9.533 13.552L15.485 7.602L14.423 6.54L8.823 12.14Z"
                                fill="#44AF1D" />
                    ...
                    <button class="flex flex-row items-center justify-center" onclick="tolakUser({{ $user->id }})">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
